define(function (require, exports, module) {
    ...
    var stateOfPage =
    {
        ...
        "flagDepth": 0, // for class: (0: class, 1: attribute, 2: propertyOfA) for relationGroup: (0: relationGroup, 1:
                        // relation, 2: propertyOfR)
    ...
    $(function () {
        ...
        //$(document).on('shown.bs.modal', anyModal, function() {  // 对任何 modal 都有效 (modal
        // 显示后在执行下面的代码，这样:visible选择器才有效)
        //  // 非法输入个数初始化 （对于 add relation 来说，modal 初始化时的 input 计数不重要，因为对其来讲真正的初始化在选择 relation 类型时）
        //  stateOfPage.countInvalidInputForModal = $(this).find('input[type=text]:visible:not([readonly])').size();
        // // :not([readonly]) 是为了不错误选择因 typeahead 插件而多出来的 input // 非法输入位置 flag 初始化
