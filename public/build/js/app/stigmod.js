define("js/app/stigmod", [ "../lib/jquery", "../lib/d3.v3", "../lib/bootstrap" ], function(require, exports, module) {
    ...
    var stateOfPage = {
        model: "",
        flagCRG: 0,
        // 0: class, 1: relationGroup
        flagDepth: 0,
        // for class: (0: class, 1: attribute, 2: propertyOfA) for relationGroup: (0: relationGroup, 1: relation, 2: propertyOfR)
        "class": "",
        // <-> relationGroup
        attribute: "",
        // <-> relation
        ...
        // <-> property
        posAddAtt: "",
        // 增加 attribute 或 relation 时 插入的位置  (attrel name 或 '@') ('@' 代表最下方的add按钮)
        dirAddAtt: 0
    };
    // Model 操作底层函数 addElemInModel removeElemInModel
    function addElemInModel(model, path, keyValue) {
        // 添加、修改元素 (addElemInModel 同时还有 modify value 的作用，但若想 modify key，则需要 modifyElemInModel)
        var len = path.length;
    ...
    function modifyElemInModel(model, path, keyOld, keyNew) {
        // modify key
        if (keyOld !== keyNew) {
            // 仅在新旧两 key 不同时才执行下面的替换操作
            var len = path.length;
            var innerModel = model[path.pop()];
            while (0 != path.length) {
                // dump_obj(innerModel);
    ...
    function removeElemInModel(model, path, key) {
        // 删除元素(keyvalue全部删除)
        var len = path.length;
        var innerModel = model[path.pop()];
        while (0 != path.length) {
    ...
    function getElemInModel(model, path) {
        // 读取元素（指定路径下的所有keyvalue）
        var len = path.length;
        var innerModel = model[path.pop()];
        while (0 != path.length) {
            // dump_obj(innerModel);
    ...
    function addClass(model, className) {
        addElemInModel(model, [ 0 ], [ className, [ {}, {
            order: []
        } ] ]);
    }
    ...
    function addRelationGroup(model, relationGroupName) {
        addElemInModel(model, [ 1 ], [ relationGroupName, [ {}, {
            order: []
        } ] ]);
    }
    ...
    function modifyRelation(model, relationGroupName, relationName, newName) {
        // TODO: 以后如果rel ID 更新了，也要同步更新order数组里的ID
        modifyElemInModel(model, [ 0, relationGroupName, 1 ], relationName, newName);
    }
    function modifyPropertyOfR(model, relationGroupName, relationName, propertyKey, newValuePair) {
        // 这个函数并不好用，因为很多时候我们只想修改 Pair 中的一个
    ...
    function elemExists(caseOfElem, name, additionalName) {
    ...
    function refreshMiddelPanelTitle(model) {
        ...
        $panels.each(function() {
            ...
            if (0 === stateOfPage.flagCRG) {
                ...
                $title.append("<span>" + middle + "</span>").find("span:last-child").css({
    ...
    function insertMiddle(model, name, noUnfold) {
        ...
        // 计算新 .panel 的编号
        var $panelTitle = $("#stigmod-cont-right .panel " + (0 === stateOfPage.flagCRG ? ".stigmod-attr-cont-middle-title" : ".stigmod-rel-cont-middle-title"));
        // 取出所有 .panel
        if (0 === $panelTitle.length) {
            // 还没有 .panel
            collapseIndex = 0;
        } else {
            // 已经有至少一个 .panel
        ...
        $compo.attr({
            "stigmod-attrel-name": name
        });
        // 设置collapse属性
        var $collapseTrigger = $compo.find(0 === stateOfPage.flagCRG ? ".stigmod-attr-cont-middle-title" : ".stigmod-rel-cont-middle-title").attr({
            "data-target": "#collapse" + collapseIndex
        });
        var $collapseContent = $compo.find(".panel-collapse").attr({
            id: "collapse" + collapseIndex
        });
    ...
    function fillLeft(model) {
        // 向左侧栏填入 class 组件和数据
        var $compo = $("#stigmod-pg-workspace #stigmod-nav-left-scroll .panel:first-child .list-group").empty();
        // 清空
        var modelClassOrdered = [];
        for (var modelClass in model[0]) {
            // 类名读入数组
            modelClassOrdered.push(modelClass);
        }
        modelClassOrdered.sort();
        // 排序
        for (var i in modelClassOrdered) {
            // 类名
            $compo.append(componentLeftClass);
            $compo.find("a:last-child > span:first-child").text(modelClassOrdered[i]).attr("stigmod-nav-left-tag", modelClassOrdered[i]);
        }
        // 向左侧栏填入 relation group 组件和数据
        $compo = $("#stigmod-pg-workspace #stigmod-nav-left-scroll .panel:last-child .list-group").empty();
        // 清空
        var modelRelationGroupOrdered = [];
        for (var modelRelationGroup in model[1]) {
            // 关系组名读入数组
            modelRelationGroupOrdered.push(modelRelationGroup);
        }
        modelRelationGroupOrdered.sort();
        // 排序
        for (var i in modelRelationGroupOrdered) {
            // 关系组名
            $compo.append(componentLeftRelationGroup);
            $compo.find("a:last-child > span:first-child").text(modelRelationGroupOrdered[i]).attr("stigmod-nav-left-tag", modelRelationGroupOrdered[i]);
        }
    }
    /// 填充中间栏为空白
    ...
    /// 填充中间栏
    function fillMiddle(model) {
        // flagCRG 标明是 Class(0) 还是 RelationGroup(1), nameCRG 是 Class 或 RelationGroup 的名字
        // 填入中间栏基本页面
        fillMiddleBasic();
        // 向中间栏填入组件和数据
        $("#stigmod-cont-right-scroll #stigmod-classname > span:nth-child(2)").text(stateOfPage.class);
        $("#stigmod-cont-right .panel").remove();
        // 清空
        var modelAttribute = model[stateOfPage.flagCRG][stateOfPage.class][1]["order"];
        // 获取 attribute 或 relation 的顺序信息
        for (var i in modelAttribute) {
            // i 既是 attrel 的编号， 也是 collapse 的序号
            ...
            // 在 .panel 中记录 attribute 或 relation 的名字，便于点击时更新 stateOfPage
            $compo.attr({
                "stigmod-attrel-name": modelAttribute[i]
            });
            // 设置collapse属性
            var $collapseTrigger = $compo.find(0 === stateOfPage.flagCRG ? ".stigmod-attr-cont-middle-title" : ".stigmod-rel-cont-middle-title").attr({
    ...
    /// 左侧、右侧栏目高度自适应
    $(function() {
        $("#stigmod-pg-workspace #stigmod-nav-left-scroll").height($(window).height() - 167 - 10 - 80);
        // 页面加载后，调整左侧导航栏高度以适应窗口大小，167为经验值，45是project标题栏高度，80是底部footer的高度
        $(window).resize(function() {
            $("#stigmod-pg-workspace #stigmod-nav-left-scroll").height($(window).height() - 167 - 10 - 80);
        });
    });
    $(function() {
        $("#stigmod-pg-workspace #stigmod-cont-right-scroll").height($(window).height() - 122 - 80);
        // 页面加载后，调整左侧导航栏高度以适应窗口大小，167为经验值，45是project标题栏高度，80是底部footer的高度
        $(window).resize(function() {
            $("#stigmod-pg-workspace #stigmod-cont-right-scroll").height($(window).height() - 122 - 80);
        });
    ...
    $(function() {
        $("#stigmod-pg-workspace #stigmod-rcmd-right-scroll").height($(window).height() - 122 - 80);
        // 页面加载后，调整左侧导航栏高度以适应窗口大小，167为经验值，45是project标题栏高度，80是底部footer的高度
        $(window).resize(function() {
            $("#stigmod-pg-workspace #stigmod-rcmd-right-scroll").height($(window).height() - 122 - 80);
        });
    ...
    $(function() {
        $(document).on("click", "#stigmod-pg-workspace #stigmod-nav-left-scroll .list-group-item", function() {
            ...
            stateOfPage.flagCRG = "stigmod-nav-left-class" === $it.attr("class") ? 0 : 1;
            // 0: class, 1: relationgroup
    ...
    $(function() {
        $(document).on("click", "#stigmod-pg-workspace #stigmod-cont-right .panel", function() {
            ...
            $collapseToggle.attr("data-toggle", "none");
            // 禁用其他panel的collapse触发器
            $collapseToggleThis.attr("data-toggle", "collapse");
            // 打开本panel的collapse触发器(下次点击即可触发)
        ...
        switch (inputCase) {
          ...
          case "class-modify":
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (stateOfPage.class !== input && elemExists(0, input)) {
                // 新类名与【其他】类名重复 (与该类修改前类名重复是允许的)
                return "Class already exists.";
            } else {
                // 合法
                return "valid";
          ...
          // attribute 名
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (elemExists(2, input, stateOfPage.class)) {
                // attribute 名重复
          ...
          case "attribute-modify":
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (stateOfPage.attribute !== input && elemExists(2, input, stateOfPage.class)) {
                // attribute 名与其他 attribute 重复
                return "Attribute name already exists.";
            } else {
                // 合法
                return "valid";
          ...
          // 类型名
            case "type-add":
          case "type-modify":
            pattern = /^(int|float|string|boolean)$/;
            // build-in types
            if (!pattern.test(input) && !elemExists(0, input)) {
                // 不是内置类型，也不是类
          ...
          case "multiplicity-modify":
            pattern = /^(\*|\d+(\.\.(\d+|\*))?)$/;
            function isValidMultiplicity(mul) {
                // 检验是否第一个数小于第二个数
                var hasTwoNum = /\.\./;
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (!isValidMultiplicity(input)) {
                // 第一个数大于第二个数
    ...
    function checkInput($input) {
        // $input 是单个输入框组件
        var inputCase = $input.attr("stigmod-inputcheck");
        // 输入框类型
        // 仅在设定了输入框的 stigmod-inputcheck 属性时进行下面的检查操作
        if (undefined !== inputCase) {
            var input = $input.val();
            // 输入框内容
            var checkResult = getInputCheckResult(inputCase, input);
            // 合法性检查结果
            ...
            switch (inputCase) {
              case "class-modify":
                ...
                // 对于修改类名来说，为防止提示被区域上沿吃掉，将提示显示在输入框之下
                break;
    ...
    function checkInputs($inputs) {
        ...
        for (var i = 0; i < num; ++i) {
            allInputsAreValid = checkInput($inputs.eq(i)) && allInputsAreValid;
        }
    ...
    $(function() {
        ...
        function enterEdit(event) {
            ...
            if ("title" === caseEdit) {
                ...
                for (var i = 0; i < num - 1; ++i) {
                    // 同时适用于单列和多列的情况 (最后一个元素是按钮，不参与循环中的处理)
                    var originalText = $originalTextElem.eq(i).text();
                    // 获取原始文字
                    switch (caseEdit) {
                      case "text":
                        // 输入框
                      ...
                      case "reltype":
                        // relation 页面的 relation type
                        if (0 === i) {
                            $editComponent.eq(i).find("button").text(originalText);
                            if ("Generalization" === originalText) {
                                // 当关系类型为Generalization时，不显示名字
                                flagGeneralization = 1;
                            }
                        } else if (1 === i) {
                            if (1 === flagGeneralization) {
                                // 当关系类型为Generalization时，不显示名字
                                $editComponent.eq(i).find("input").css({
                                    display: "none"
                                });
                                flagGeneralization = 0;
                            } else {
                                $editComponent.eq(i).find("input").val(originalText);
                            }
                        }
        ...
        function submitEdit(event) {
            ...
            var $originalTextElem = $root.find(".stigmod-clickedit-disp");
            var $editComponent = $root.find(".stigmod-clickedit-edit");
            var $visibleInputs = $root.find("input[type=text]:visible:not([readonly])");
            // :not([readonly]) 是为了屏蔽 typeahead 插件的影响
            ...
            if (checkInputs($visibleInputs)) {
                ...
                if ("title" === caseEdit) {
                        ...
                        for (var nameA in model[0][nameC][0]) {
                            if (originalTitle === model[0][nameC][0][nameA][0]["type"]) {
                                ...
                                if (nameC === stateOfPage.class) {
                                    ...
                                    $thePanel.find("tr.stigmod-attr-prop-type > td:nth-child(2) > span:nth-child(1)").text(newTitle);
                                    // 更新相关的 attribute 的 type 的值
                                    refreshMiddelPanelTitle(model);
                                }
                            }
                        }
                    ...
                    var relationGroups = getElemInModel(model, [ 1 ]);
                    // 获取所有 relation group
                    for (var nameRG in relationGroups) {
                        // 遍历该 model 中的所有 relation group
                        eval("var matchName = nameRG.match(/\\b" + originalTitle + "\\b/)");
                        if (null !== matchName) {
                            // 如果该 relation group 与被修改的 class 有关
                            eval("var newNameRG = nameRG.replace(/\\b" + originalTitle + '\\b/g, "' + newTitle + '")');
                            ...
                            for (var nameR in relationGroups[nameRG][0]) {
                                // 遍历该 relation group 中的所有 relation
                                // 修改 relation 中的 class name
                                var nameClass = relationGroups[nameRG][0][nameR][0]["class"];
                                // 获取该 relation 两端的 class 的名字的 引用
                                if (originalTitle === nameClass[0]) {
                                    // 需要修改End0
                        ...
                        switch (caseEdit) {
                          ...
                          case "reltype":
                            ...
                            if (0 === i) {
                                ...
                                if ("Generalization" === newText) {
                                    // 当关系类型为Generalization时，将name置空
                                    flagGeneralization = 1;
                                }
                            } else if (1 === i) {
                                if (1 === flagGeneralization) {
        ...
        $(document).on("click", ".stigmod-clickedit-btn-edit", enterEdit);
        // “编辑”按钮的点击编辑功能
        $(document).on("click", ".stigmod-clickedit-btn-ok", submitEdit);
        // 编辑组件内的“提交”按钮功能
        $(document).on("click", ".stigmod-clickedit-btn-cancel", cancelEdit);
    });
    // ajax 测试
    $(function() {});
    // 远程获取 html 测试
    ...
    $(function() {
        $(document).on("show.bs.dropdown", ".stigmod-hovershow-trig", function() {
            ...
            // 不从stateOfPage获取attribute的原因是bootstrap的dropdown不好设置timeout，而没有timeout就不能保证stateOfPage的状态是最新的。
    ...
    $(function() {
        $(document).on("click", ".stigmod-dropdown-addprop .dropdown-menu a", function(event) {
            ...
            setTimeout(function() {
                // 延时是为了解决 stateOfPage 还没有更新就使用未更新的 stateOfPage.attribute 值的问题
                if (0 === stateOfPage.flagCRG) {
            ...
            // 进入编辑状态
            $propertyRow.find("input[type=radio][value=True]").prop("checked", true);
            // 单选框都默认勾选 True
        ...
        $(document).on("click", "#stigmod-dropdown-reltype-modal a", function(event) {
            ...
            switch (reltype) {
              case "Generalization":
                ...
                // .tooltip('destroy') 的作用是去掉可能存在的非法提示
                $roleModify.eq(0).val("father").tooltip("destroy");
                $roleModify.eq(1).val("child").tooltip("destroy");
                $multiplicityModify.eq(0).attr({
                    disabled: ""
                }).val("1").tooltip("destroy");
                $multiplicityModify.eq(1).attr({
                    disabled: ""
                }).val("1").tooltip("destroy");
                break;
    ...
    $(function() {
        ...
        $(document).on("click", "#stigmod-btn-addrelationgroup", function() {
            var $inputs = $(this).closest("#stigmod-modal-addrelationgroup").find("input[type=text]:not([readonly])");
            // :not([readonly]) 是为了屏蔽 typeahead 插件的影响
    ...
    $(function() {
        $(document).on("click", ".stigmod-addattrel-trig", function(event) {
            var $this = $(this);
            // 为了在 setTimeout() 函数中仍能正却使用
            setTimeout(function() {
                // 延时是为了解决 stateOfPage 还没有更新 modal 就弹出的问题
                // 获取添加位置和方向信息（也可写在 setTimeout() 之外）
                if ($this.hasClass("stigmod-addattrel-last")) {
                    // 在add大按钮的上方添加（即所有panel的末尾，可能还没有panel）
                    stateOfPage.posAddAtt = "@";
                    stateOfPage.dirAddAtt = 0;
                } else {
                    stateOfPage.posAddAtt = $this.closest(".panel").attr("stigmod-attrel-name");
                    if ($this.hasClass("stigmod-addattrel-above")) {
                        // 向上添加
                        stateOfPage.dirAddAtt = 0;
                    } else {
                        // 向下添加
    ...
    $(function() {
        $(document).on("click", "#stigmod-btn-addattribute", function() {
            var $visibleInputs = $(this).closest("#stigmod-modal-addattribute").find("input[type=text]:visible:not([readonly])");
            // :not([readonly]) 是为了屏蔽 typeahead 插件的影响
            if (checkInputs($visibleInputs)) {
                // 添加 attribute 名
    ...
    $(function() {
        ...
        $(document).on("click", "#stigmod-btn-addrelation", function() {
            var $visibleInputs = $(this).closest("#stigmod-modal-addrelation").find("input[type=text]:visible:not([readonly])");
            // :not([readonly]) 是为了屏蔽 typeahead 插件的影响
    ...
    /// 所有删除按钮的入口
    $(function() {
        $(document).on("click", ".fa-remove, .glyphicon-trash, .stigmod-remove-trig", function() {
            // TODO：直接写图标类不是长久之计
            setTimeout(function() {
                // 延时是为了解决 stateOfPage 还没有更新 modal 就弹出的问题
    ...
    $(function() {
        $(document).on("click", "#stigmod-btn-remove", function() {
            // TODO: 删除后，stateOfPage的更新。
            switch (stateOfPage.flagDepth) {
              case 0:
                // 修改 model
                removeElemInModel(model, [ stateOfPage.flagCRG ], stateOfPage.class);
                if (0 === stateOfPage.flagCRG) {
                    // 删除 class 时，还要删除与之相关的 relation group
                    var relationGroups = getElemInModel(model, [ 1 ]);
                    // 获取所有 relation group
                    for (var nameRG in relationGroups) {
                        // 遍历该 model 中的所有 relation group
                        eval("var matchName = nameRG.match(/\\b" + stateOfPage.class + "\\b/)");
              ...
              case 1:
                ...
                var order = model[stateOfPage.flagCRG][stateOfPage.class][1]["order"];
                // 获取顺序
                order.splice(order.indexOf(stateOfPage.attribute), 1);
                // 从顺序数组中删除对应的 attrel 项
                // 更新显示
    ...
    $(function() {
        ...
        $(document).on("show.bs.modal", anyModal, function() {
            // 对任何 modal 都有效
            $(this).find(".tooltip").remove();
        });
        $(document).on("shown.bs.modal", anyModal, function() {
            // 对任何 modal 都有效
            focusOnInputIn($(this));
        });
        ...
        $(document).on("show.bs.modal", "#stigmod-modal-addattribute", function() {
            ...
            $(this).find("input[value=type]").prop("checked", true);
            // 保留type项的选中状态
            $(this).find("tr").hide();
            $(this).find("tr:nth-child(1)").css("display", "table-row");
            // 显示name项
            $(this).find("tr:nth-child(2)").css("display", "table-row");
        });
        $(document).on("show.bs.modal", "#stigmod-modal-addrelation", function() {
            ...
            // 单选框都默认勾选 True
            $(this).find("input[type=checkbox]").removeAttr("checked");
            $(this).find("input[value=role]").prop("checked", true);
            // 保留role项的选中状态
            $(this).find("input[value=multiplicity]").prop("checked", true);
            // 保留multiplicity项的选中状态
            $(this).find("tr").hide();
            ...
            var nameOfBothEnds = stateOfPage.class.split("-");
            // 获得关系两端的类名
            $(this).find("tr:nth-child(3) > td:nth-child(2) > input").val(nameOfBothEnds[0]);
            // 将类名填入
            $(this).find("tr:nth-child(3) > td:nth-child(3) > input").val(nameOfBothEnds[1]);
            // 将类名填入
            $(this).find("tr:nth-child(1)").css("display", "table-row");
            // 显示type项
            $(this).find("tr:nth-child(2)").css("display", "table-row");
            // 显示role项
            $(this).find("tr:nth-child(3)").css("display", "table-row");
            // 显示class项
            $(this).find("tr:nth-child(4)").css("display", "table-row");
        });
    ...
    +function d3view() {
        ...
        var svg = d3.select("#stigmod-d3view-left").append("svg").attr("width", w).attr("height", h);
        //Initialize a default force layout, using the nodes and edges in dataset
        var force = d3.layout.force().nodes(dataset.nodes).links(dataset.edges).size([ w, h ]).linkDistance(100).charge([ -400 ]).start();
        var colors = d3.scale.category10();
        var link = svg.selectAll(".link").data(dataset.edges).enter().append("g").attr("class", "link");
        link.append("line").style("stroke", "#ccc").style("stroke-width", 1);
        /*link.append("text")
     ...
     .text("0");*/
        ...
        var node = svg.selectAll(".node").data(dataset.nodes).enter().append("g").attr("class", "node").on("mouseover", mouseover).on("mouseout", mouseout).on("click", function(d) {
            d3.select(".property").remove();
            d3.select("#stigmod-d3view-right").append("div").attr("class", "property");
            d3.select(".property").append("br");
            d3.select(".property").append("p").attr("style", "padding-left:10%;font-weight:bold").text("Class:");
            d3.select(".property").append("p").attr("style", "padding-left:20%").text(d.name);
            d3.select(".property").append("br");
            d3.select(".property").append("p").attr("style", "padding-left:10%;font-weight:bold").text("Properties:");
            d.property.forEach(function(property) {
                d3.select(".property").append("p").attr("style", "padding-left:20%").text(property.name);
            });
        }).call(force.drag);
        ...
        function mouseover() {
            d3.select(this).select("circle").transition().duration(500).ease("bounce").attr("r", function(d) {
                //设置圆点半径
                return radiuson(d);
